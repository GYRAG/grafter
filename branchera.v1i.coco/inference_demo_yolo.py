#!/usr/bin/env python3
"""
Inference demo script for the trained YOLOv8 branch detection model.
...
class BranchDetectorYOLO:
    """Branch detection inference class using YOLOv8."""
    
    def __init__(self, model_path="output/branch_detection/weights/best.pt"):
    ...
    def process_image(self, image_path, confidence_threshold=0.5, save_output=True, output_dir="./output/inference"):
        ...
        # Detect branches
        results, result = self.detect_branches(image_path, confidence_threshold)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="YOLOv8 Branch Detection Inference Demo")
    ...
    parser.add_argument("--model", type=str, default="output/branch_detection/weights/best.pt", help="Path to trained model")
    
    args = parser.parse_args()
    
    print("🌿 YOLOv8 Branch Detection Inference Demo")
    print("=" * 50)
    
    try:
        # Create detector
        detector = BranchDetectorYOLO(args.model)
        detector.load_model()
